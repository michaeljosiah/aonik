...
> ⚠️ **Current status — the server gate has landed and fails closed; frontend/voice presentation is still pending.** `IToolApprovalGate` (`ToolApprovalGate`, registered in `AgentsModule.cs`) wraps every classified mutating tool in `ApprovalGatedAIFunction` — our *enforcing* decorator, not the framework's passive `ApprovalRequiredAIFunction` marker — and an unclassified mutating-looking tool throws `ToolNotClassifiedException` at build time. The Finance agent's tools route through `gate.GateAll(...)` in `FinanceAgentRegistration` and are classified by `FinanceToolApprovalManifest`: the money tools (`finance_capture_payment`, `finance_cancel_payment`, `finance_create_payment_intent`, `finance_mark_invoice_paid`) are **high** → marshalled into a durable `Proposal`, never run in-band; the everyday invoice/ledger writes are **medium** → a durable, args-hash-bound `ToolApprovalRequest` runs the tool inline **once**, only after a human approves via `POST /ai/tool-approvals/{id}/decide`.
>
> **Still pending:** the Admin UI does not yet render the medium-approval card or route approve/reject through `DecideAsync`, and voice parity is outstanding. The server boundary is real regardless of the frontend, so the remaining work is UX, not enforcement. See `CLAUDE.md` for the fuller statement.
