...
function AonikMark({ size = 22 }) {
  return (
    <span style={{ position: 'relative', display: 'inline-block', width: size, height: size }}>
      ...
      <span style={{
        position: 'absolute',
        top: Math.round(size * 0.1), right: Math.round(size * 0.1),
...
// ─── Pill ────────────────────────────────────────────────────────────────
function Pill({ tone = 'default', children, dot = false }) {
  const cls = {
    default: 'pill',
    tint: 'pill pill-tint',
  ...
  return (
    <span className={cls}>
      {dot && <span style={{
        width: 6, height: 6, borderRadius: 999, background: 'currentColor', flex: 'none'
      }}/>}
      {children}
    </span>
  );
