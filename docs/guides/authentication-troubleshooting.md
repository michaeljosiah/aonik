...
- [ ] Token has `email` claim (required for AONIK user identification)
...
| User not found | Check email claim in token, then wait for JIT provisioning on first API call |
| Email claim missing | Add email claim to Auth0 Action: `api.accessToken.setCustomClaim('email', event.user.email);` |
