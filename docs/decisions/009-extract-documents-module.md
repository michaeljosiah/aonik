...
**Status**: Accepted — Phases 0–4 landed (contracts + party-scoped vector index, module extraction, Compliance decouple, event-driven RAG ingestion, scoped agent search, legacy decommission + opt-in backfill). The deletion / right-to-erasure path (§15) is the remaining follow-up.
...
| 4 | ✅ Landed (deletion pending) | Event-driven ingestion via an outbox `IEventHandler<DocumentUploadedEvent>` in the Worker (durable retry/back-off/dead-letter) + native/DOCX text extraction with a deferred `IDocumentOcrExtractor` hook; `IDocumentSearch` exposed as the read-only `documents_search` agent tool (scope from auth, never model input); legacy `DocumentUploadEndpoint` removed; opt-in `DocumentIngestionBackfillJob` (disabled by default); `Document-Model.md` updated. **Remaining:** the deletion / right-to-erasure path (§15 — blob + vector purge, `DocumentDeletedEvent` orphan handling). |

> **Toolchain note.** Phase 0 landed first as an entirely additive change (new SharedKernel files + one Infrastructure adapter + DI wiring) so it neither moved entities nor touched the migration stream — safe without a local build. Phases 1–4 then landed once the .NET toolchain was available to build and run `dotnet ef`, with the entity-move migration **tool-generated** against `AonikDbContext` per CLAUDE.md (never hand-authored). The single deferred item is the deletion/erasure path; the `PurgeDocumentAsync` mechanism and `DocumentDeletedEvent` already exist, so it is a wiring task, not new infrastructure.
