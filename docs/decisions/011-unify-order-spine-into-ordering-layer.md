...
| Phase | Description |
|-------|-------------|
| 1 | **Generalise in place.** Add the nullable retail columns to `OrderItem`, add `ProductPurchase` to `OrderType`, update the doctrine. One tool-generated migration against `AonikDbContext`. No relocation yet — Finance's `Order` is now retail-capable. Lowest-risk, fully reversible, proves the "just another order type" thesis against the real schema. |
