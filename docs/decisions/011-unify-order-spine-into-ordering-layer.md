...
**Status**: Accepted — Phases 1–3 landed (generalise in place; contract seam; spine relocated to Aonik.Ordering); Phase 4 (Aonik.Commerce) pending
**Date**: 2026-06-17
...
2. **The Order entities live in the middle layer.** The anemic `Order`, `OrderItem`, `OrderPartyRole`, `OrderFundingRef`, `OrderFulfilmentRef`, `OrderHistoryEvent`, `OrderNote` entities **and their EF configurations** move into **`Aonik.Ordering`** (entity placement decided during implementation — see Phase 3), so the entity travels with its service and `SharedKernel` stays lean (only the contract lands in the kernel). Their C# namespaces are **preserved** (`Aonik.Finance.Entities.Orders`) so the EF model-snapshot FQN strings stay valid and the relocation needs **no migration**. The one exception is `OrderFulfilmentRefConfiguration`, whose FK wiring to Finance payment entities (`Payout`/`PaymentIntent`/`PartnerBillPayment`) keeps it in Finance — the generic entity does not know those Finance types.
...
| 3 ✅ | **Relocate the spine (landed).** Created `Aonik.Ordering` (references only SharedKernel) and moved the Order entities + EF configs + the core `OrderService` impl into it via `git mv`, **C# namespaces preserved** (`Aonik.Finance.Entities.Orders`) so the EF snapshot FQN strings stay valid — verified by an **empty probe-migration diff**, no schema change. Added `OrderingDbContext` (scoping, no migrations) and `OrderingModule` (registered in the composition root). `Aonik.Infrastructure` and `Aonik.Finance` reference `Aonik.Ordering` downward; both `AonikDbContext` and `FinanceDbContext` apply the Ordering assembly's Order configs. The `OrderFulfilmentRef` **entity** lives in Ordering but its FK config to Finance payment entities (`Payout`/`PaymentIntent`/`PartnerBillPayment`) stays in Finance. Finance keeps only type-specific orchestration. Entity placement chosen: **in `Aonik.Ordering`** (not SharedKernel), keeping the kernel lean. Lifecycle endpoints + agent tools relocation deferred (they still compile in Finance via the downward reference). Build green; Architecture 13/13, Application 1088, Api 242 pass. |
