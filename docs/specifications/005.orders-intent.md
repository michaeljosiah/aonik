...
- `Order` (module-aligned entity under `Aonik.Domain.Orders`).
...
- `Invoice` linkage for billable intent (one order -> zero/one invoice).

## Order Types
- `BillPayment`
- `BankTransfer`
- `CashCollection`
- `ProductPurchase` — a purchase of goods (see [ADR-011](../decisions/011-unify-order-spine-into-ordering-layer.md) / [Spec 041](041.unified-order-spine-ordering-layer.html)). The retail line shape lives on `OrderItem` (`Quantity`, `UnitPrice`, `ProductId` soft-ref, `Sku`); `AmountIn` carries the line total. Funding (`PaymentIntent`) and billing (`Invoice`) are unchanged.
- Additional types can be added without changing base order fields. An Order is the canonical record of a customer's intent to **transact** — a financial service *or* a product purchase — with `OrderType` discriminating the nature; this generalises the original "financial service" framing per ADR-011.
...
- Assign immutable identifiers: orderId, orderNumber, invoiceId (when billing applies).
- Persist type-specific data in `OrderDetailsJson` (opaque JSON).
...
- When multiple payable elements exist, create `OrderItems` with itemType + reference + amount + currency.
...
- Include invoice status when invoice exists.
...
- Order stores `OrderDetailsJson` as the canonical type-specific payload.
- OrderItems are created when multiple payable elements exist.
...
- Orders link to PaymentIntents, Payouts, Invoices, and ledger references (not vice versa).
...
- `POST /orders`
  - Request: `customerId`, `orderType`, `serviceCode`, `amount`, `currency`, `pricingQuoteId`,
             ...
             `payer?`, `payee?`, `details`, `items?`, `metadata?`
  - Response: `orderId`, `orderNumber`, `invoiceId`, `status`, `createdAt`, `paymentStatus?`, `invoiceStatus?`
- `GET /orders/{orderId}`
  - Response: `orderId`, `orderNumber`, `invoiceId`, `status`, `paymentStatus`,
              `invoiceStatus?`, `orderType`, `serviceCode`, `details`, `items`, `amounts`, `fees`, `fx`,
...
- Persist `OrderDetailsJson` from typed details payload.
- Create `OrderItems` when multiple payable elements are present.
...
OrderService -> Billing: Create invoice (when billable)
OrderService -> Payment: Create PaymentIntent (OrderId + InvoiceId)
...
sequenceDiagram
    ...
    OrderSvc->>Billing: Create invoice when billable
    OrderSvc->>Payments: Create PaymentIntent (OrderId + InvoiceId)
