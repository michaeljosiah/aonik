...
internal sealed class WorkspaceBlobSweepJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var gate = await ModuleGatedTenants.FilterAsync(
            _moduleReader, tenants, ModuleIds.Workspaces, "Workspace blob sweep", _logger, context.CancellationToken);
        ...
        await TenantScopedJob.ForEachTenantAsync(
            _tenantContext, gate.Enabled, "workspace-blob-sweep",
            async ct =>
            {
        ...
        context.Result = $"Blobs deleted {deleted}, claims abandoned {abandoned}." + gate.Note;
    }
}
