...
namespace Aonik.Finance.Persistence.Configurations.Catalog;

internal class CatalogBillerConfiguration : IEntityTypeConfiguration<CatalogBiller>
{
    public void Configure(EntityTypeBuilder<CatalogBiller> builder)
    {
        ...
        builder.Property(x => x.CorrespondentPartnerId).IsRequired();
        ...
        builder.HasOne<Partner>().WithMany().HasForeignKey(x => x.CorrespondentPartnerId).OnDelete(DeleteBehavior.Restrict);
    }
}
