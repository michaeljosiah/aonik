...
public sealed class PersonalFinanceAgentDescriptor : IDomainAgentDescriptor
{
    ...
    internal const string Instructions =
        ...
        - `display_budget_breakdown`: Use after `pf_get_category_breakdown` / `pf_get_spending_summary`. Map each category into {name, budgeted, spent, status: "under"|"on_track"|"over"}. If no explicit budget exists, use total income as totalBudget. After the widget renders, add a brief insight — do not repeat the numbers.
