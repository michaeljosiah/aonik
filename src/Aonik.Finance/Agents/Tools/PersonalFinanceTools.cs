...
internal sealed class PersonalFinanceTools
{
    ...
    // ── AONIK Compass Tools (Spec 021) ────────────────────────────
    //
    // Goal programme management, plan generation/retrieval, deterministic
    // safe-to-spend, and Compass proposal creation. Read tools execute
    // directly; mutating tools are gated server-side (Spec 032) and follow the
    // confirmAction convention. Compass never moves money — recommendations are
    // Proposals reviewed by the user.
    ...
    public static IEnumerable<AITool> CreateAll(IServiceProvider serviceProvider)
    {
        ...
        yield return AIFunctionFactory.Create(tools.ListGoals, name: "pf_list_goals");
        yield return AIFunctionFactory.Create(tools.GetGoal, name: "pf_get_goal");
        yield return AIFunctionFactory.Create(tools.GetGoalPlan, name: "pf_get_goal_plan");
        yield return AIFunctionFactory.Create(tools.GetSafeToSpend, name: "pf_get_safe_to_spend");
        yield return AIFunctionFactory.Create(tools.RunCompassPlanner, name: "pf_run_compass_planner");
        ...
        yield return AIFunctionFactory.Create(tools.CreateGoalProgramme, name: "pf_create_goal_programme");
        yield return AIFunctionFactory.Create(tools.UpdateGoalProgramme, name: "pf_update_goal_programme");
        yield return AIFunctionFactory.Create(tools.GenerateGoalPlan, name: "pf_generate_goal_plan");
        yield return AIFunctionFactory.Create(tools.CreateCompassProposal, name: "pf_create_compass_proposal");
    }
