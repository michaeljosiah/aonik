...
export function Chip({ icon: Icon, dense = false, className, children }: ChipProps) {
  return (
    ...
    >
      {Icon && <Icon size={dense ? 9 : 10} className="text-muted-foreground" />}
      {children}
    </span>
  );
}
...
export function InfoNote({ icon: Icon = Info, children, className }: InfoNoteProps) {
  return (
    <div
      className={cn(
        'flex items-start gap-2 rounded-lg border border-dashed border-border bg-muted px-3 py-2 text-[11.5px] leading-relaxed text-muted-foreground',
        className,
      )}
    >
      <Icon size={13} className="mt-px flex-none text-muted-foreground" />
      <span>{children}</span>
    </div>
  );
}
...
export function EmptyState({ icon: Icon, title, description, action, className }: EmptyStateProps) {
  return (
    ...
    >
      ...
      {/* Arbitrary max-width: this project's Tailwind theme mis-resolves the named
          `max-w-*` container scale (e.g. `max-w-md` computes to 16px), so the
          literal value is used to keep the copy block at a sane reading width. */}
      <div className="w-full max-w-[28rem] space-y-1">
