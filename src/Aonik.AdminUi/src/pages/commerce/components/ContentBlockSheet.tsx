  ...
  return (
    <Sheet open onOpenChange={(open) => !open && !saving && onClose()}>
      <SheetContent size="md">
            ...
            {preparation.length === 0 ? (
              ...
              <ul className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1">
                {preparation.map((line) => (
                  <li key={line.group} className="text-[12px] text-muted-foreground">
                    {line.group}:{' '}
                    <span className="text-foreground">{line.choice}</span>
                  </li>
                ))}
              </ul>
            )}
        ...
        <SheetFooter>
          <span className="mr-auto max-w-[280px] text-[11px] text-muted-foreground">
