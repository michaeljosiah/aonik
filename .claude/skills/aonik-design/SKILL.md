...
1. **Semantic tokens only.** Categorical series use `--chart-1`…`--chart-10`; brand accents are `--mark-dot` (logo only), `--agent-team`, `--agent-enterprise`. The legacy `--color-*` aliases are deleted; CI fails on any `var(--color-*)`. Use `bg-background`, `text-foreground`, `text-muted-foreground`, `bg-muted`, `bg-accent`, `border-border`, `border-input`, `ring-ring`, `bg-primary`/`text-primary-foreground`, `bg-destructive`, and the status trios `success|warning|info` (`-foreground` for text on the `-subtle` tint). Never hardcode hex or raw Tailwind palette colours.
...
8. **Guardrails are enforced.** `npm run check:design -- --strict` and `npm run check:contrast` (text 4.5:1, non-text 3:1 incl. every chart hue, both themes) run in CI. The playbook for which primitive does which job is `src/Aonik.AdminUi/src/components/ui/README.md`.
