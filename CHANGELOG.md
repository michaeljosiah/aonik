...
### Fixed
- **Infrastructure (Azure IaC/CD)**: Made Azure Container Registry SKU configurable (`Basic`/`Standard`/`Premium`) and added deployment-time SKU fallback retries (`containerRegistrySku`) in the GitHub Actions workflow to automatically handle `SkuNotSupported` across subscription/region constraints.
