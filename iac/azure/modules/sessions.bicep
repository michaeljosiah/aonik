...
// ACA Dynamic Sessions auth (https://learn.microsoft.com/en-us/azure/container-apps/sessions-code-interpreter#authentication):
//   "Valid Microsoft Entra tokens are generated by an identity belonging to
//    the Azure ContainerApps Session Executor AND Contributor roles on the
//    session pool."
// First iteration of this bicep only granted Session Executor; ACA's
// /executions endpoint returned HTTP 401 to AcaSessionsClient even though
// the token had the right audience. Adding Contributor satisfies the
// "both roles" requirement.
